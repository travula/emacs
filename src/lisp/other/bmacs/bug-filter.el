...
;*    Last change :  Tue Jan 28 11:12:09 2003 (serrano)                */
...
(defun bug-filter-location (proc output)
  (if (string-match bug-linespec-regexp output)
	...
	(bug-prompt-file-line f (string-to-int l)))))
...
(defun bug-filter-bp (proc output)
  (if (string-match bug-breakpoint-regexp output)
	...
	(bug-breakpoint 'bp (string-to-int n) f c (string-to-int l) (intern s)))))
...
(defun bug-filter-fp (proc output)
  (if (string-match bug-breakpoint-regexp output)
      (let ((n (substring output (match-beginning 1) (match-end 1)))
	    ...
	    (l (substring output (match-beginning 4) (match-end 4)))
	    (s (substring output (match-beginning 5) (match-end 5))))
	(bug-breakpoint 'fp (string-to-int n) f c (string-to-int l) (intern s)))))
  ...
  #'(lambda (e)
	...
	(if (consp l)
	    (let ((bp (car (cdr l))))
		  ...
		  (let ((n (string-to-int (match-string-no-properties 1 bp))))
  ...
  #'(lambda (e)
	...
	(if (consp l)
	    (let ((f (car (cdr l))))
		  ...
		  (let ((n (string-to-int (match-string-no-properties 1 f))))
